...
NOUZ предоставляет 15 инструментов через Model Context Protocol. Доступность зависит от режима:

| # | Инструмент | Что делает | LUCA | PRIZMA | SLOI |
|---|-----------|------------|------|--------|------|
| 1 | `read_file` | читает файл | ✓ | ✓ | ✓ |
| 2 | `write_file` | записывает файл | ✓ | ✓ | ✓ |
| 3 | `list_files` | показывает список | ✓ | ✓ | ✓ |
| 4 | `get_children` | показывает потомков | ✓ | ✓ | ✓ |
| 5 | `get_parents` | показывает родителей | ✓ | ✓ | ✓ |
| 6 | `format_entity_compact` | строит формулу узла | ✓ | ✓ | ✓ |
| 7 | `index_all` | переиндексирует базу | ✓ | ✓ | ✓ |
| 8 | `suggest_metadata` | предлагает разметку | — | ✓ | ✓ |
| 9 | `suggest_parents` | предлагает родителей | — | ✓ | ✓ |
| 10 | `embed` | строит embedding | — | ✓ | ✓ |
| 11 | `calibrate_cores` | пересчитывает эталоны | — | ✓ | ✓ |
| 12 | `recalc_signs` | пересчитывает знаки | — | ✓ | ✓ |
| 13 | `recalc_core_mix` | пересчитывает доменный состав | — | ✓ | ✓ |
| 14 | `process_orphans` | разбирает сироты | — | ✓ | ✓ |
| 15 | `add_entity` | создаёт сущность | — | ✓ | ✓ |
...
Прочитать Markdown-файл и вернуть YAML-блок вместе с текстом. В YAML используются поля `type`, `level`, `sign`, `artifact_sign`, `parents`, `tags`. При чтении файл переиндексируется в базе.

### `write_file`

Создать или обновить заметку с YAML-блоком. Перед записью проверяет граф на циклы и синхронизирует обычные родительские ссылки `parents` с расширенным описанием связей `parents_meta`.
