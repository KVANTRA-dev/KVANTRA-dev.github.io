...
Так агент работает не только с текстом заметки, но и с её положением в базе. Семантические расчёты идут отдельно: текст сравнивается с эталонами, мосты ищутся по эмбеддингам, а `core_mix` показывает, как содержание заметок меняет общую картину снизу вверх.
...
| L4 Квант | Опциональный составной знак: собственный `artifact_sign` + доменный `sign` по содержимому |
...
NOUZ хранит индекс в SQLite (`obsidian_kb.db` в корне папки базы, `vault`):
