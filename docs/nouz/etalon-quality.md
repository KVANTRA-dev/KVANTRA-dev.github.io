...
При **классификации заметок** сервер сравнивает embedding заметки с mean-centered эталонами и затем считает spread-normalized проценты.
...
Если меняете тексты эталонов или embedding-модель, цифры должны быть пересчитаны. Старые cosine уже не доказывают качество нового набора.
